  ...
  <div>
    <v-row>
      <v-col cols="12" sm="6" md="4" lg="10"
        ><span class="title-page">User Access</span></v-col
      >
    ...
    <v-card class="mt-4">
      ...
      <v-card-text>
        ...
        >
          <template #[`item.accessAt`]="{ item, index }">
            <span>{{ getDate(item.accessAt) }}</span>
          </template>
          <template #[`item.note`]="{ item, index }">
            <span v-if="item.note !== null">{{ getStr(item.note) }} ...</span>
          </template>
        </v-data-table>
...
import ModalUserAccess from "./ModalUserAccess";
import UserAccessService from "../../../services/management/user_access/userAccessServices";

const service = UserAccessService.build();
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Action Name", value: "actionName" },
        { text: "Date", value: "accessAt" },
  ...
  methods: {
    ...
    async getUserId() {
      const res = await service.getUserId();
