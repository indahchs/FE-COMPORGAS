  ...
  <div style="margin-top: -80px">
    <v-container fluid class="pa-6 grey lighten-4" style="min-height: 100vh;">
      <v-card class="pa-6" rounded="lg" elevation="1">
        ...
        <v-row v-else>
          <v-col v-for="(item, index) in displayedItems" :key="index" cols="12" sm="6" md="4" lg="3" xl="3">
            <v-card hover class="elevation-2 folder-card" rounded="lg" @click="openFolderPhotos(item)">
              <v-img :src="item.image" height="200" cover>
                <v-btn v-if="canEditFolder" icon small @click.stop="openEditFolderDialog(item)" class="edit-btn" color="white" elevation="2">
                  <v-icon color="primary">{{ icons.mdiPencil }}</v-icon>
                </v-btn>
                <v-btn v-if="canDeleteFolder" icon small @click.stop="confirmDeleteFolder(item)" class="delete-btn" color="white" elevation="2">
                  <v-icon color="red">{{ icons.mdiDelete }}</v-icon>
                </v-btn>
              </v-img>
              <v-card-text class="pa-3 card-content-fixed">
                <div class="text-subtitle-2 font-weight-bold folder-name mb-1" :title="item.title">{{ item.title }}
                </div>
                ...
                <div class="text-caption folder-meta mb-1" v-if="item.location" :title="item.location">
      ...
      <!-- Edit Folder Dialog -->
      <v-dialog v-model="editFolderDialog" max-width="500">
        <v-card>
          <v-card-title>Edit Folder</v-card-title>
          <v-card-text>
            <v-form ref="editFolderForm" v-model="editFolderFormValid">
              <v-text-field v-model="editFolder.folderName" label="Folder Name"
                :rules="[v => !!v || 'Folder name is required']" outlined required></v-text-field>

              <v-select v-model="editFolder.officeLocationId" :items="locationOptions" item-text="text"
                item-value="value" label="Office Location" :rules="[v => !!v || 'Office location is required']" outlined
                required :prepend-inner-icon="icons.mdiMapMarker" :loading="isLoadingDivisions"
                :disabled="isLoadingDivisions">
                <template v-slot:selection="{ item }">
          ...
          <v-card-actions>
            ...
            <v-btn color="primary" @click="submitEditFolder" :loading="editingFolder" :disabled="!editFolderFormValid">
              Save
            </v-btn>
          </v-card-actions>
        </v-card>
...
<script>
import { mdiCamera, mdiFolderPlus, mdiClose, mdiRefresh, mdiImageOff, mdiUpload, mdiDelete, mdiCheck, mdiMagnify, mdiInformation, mdiMapMarker, mdiAccount, mdiImageMultiple, mdiPencil } from '@mdi/js'
...
export default {
  ...
  methods: {
    ...
    openEditFolderDialog(folder) {
      ...
      this.$nextTick(() => {
        if (this.$refs.editFolderForm) {
          this.$refs.editFolderForm.resetValidation()
        }
      })
    },
    ...
    async submitEditFolder() {
      ...
      try {
        ...
        this.showError('Failed to update folder')
      } finally {
