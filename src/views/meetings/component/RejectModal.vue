        ...
        <v-card-text>
          <v-row>
            <v-col cols="12" style="color: #000000">
              Are you sure rejected this book?
            </v-col>
            <v-col cols="12">
              <v-textarea
                hide-details
...
import MeetingService from "@/services/management/meeting_room/meetingRoomServices";
import { hitAPI } from "@/services/upload/uploadServices";
const postMeeting = MeetingService.build();
export default {
  ...
  methods: {
    async submit() {
      const param = {
        rejectionNote: this.desc,
      };
      const res = await postMeeting.rejectMeeting(this.datasReject.id, param);
