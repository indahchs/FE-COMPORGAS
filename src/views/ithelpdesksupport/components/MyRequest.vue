  ...
  <div>
    ...
    <v-card class="mt-4">
      ...
      <v-row style="padding: 1%">
        <v-data-table hide-default-footer :loading="loading" style="width: 100%" :headers="headers" :items="tickets"
          ...
          <template #[`item.statusName`]="{ item }">
            <span class="pa-2 status-badge" :class="getStatusClass(item.statusId)">
              {{ item.statusName }}
              <v-icon color="white">{{ getStatusIcon(item.statusId) }}</v-icon>
            </span>
          </template>
        </v-data-table>
      </v-row>
