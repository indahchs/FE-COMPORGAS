...
export default {
      ...
      listItems: [
        {
          ...
          text: "Request Ticket",
          subtitle:
            "Requests for assistance or problem reports sent to the helpdesk team",
        ...
        },
        {
          ...
          text: "Master Catalog",
          subtitle:
            "A page containing a collection of lists of available IT services",
  ...
  methods: {
    getMenu() {
      ...
      const filter = list.filter((value) => value.id !== 1);
      this.listItems = data.roleId === "IT" ? filter : this.listItems;
    },

    navigateToPage(page) {
      this.$router.push(`/ithelpdesksupport/${page}`).catch(err => {
